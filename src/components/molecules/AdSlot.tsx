...
function AdPlaceholder({ variant, className }: { variant: AdSlotVariant; className?: string | undefined }) {
  const dims = SLOT_DIMENSIONS[variant];
  return (
    <div
      className={cn('bg-slate-100 dark:bg-slate-800/50 border border-dashed border-slate-200 dark:border-slate-700 rounded-lg flex items-center justify-center', className)}
...
export function AdSlot({ adSlot, variant = 'rectangle', className, showLabel = true }: AdSlotProps) {
...
export function AdSlotLeaderboard({ className }: { className?: string | undefined }) {
  return <AdSlot adSlot={process.env['NEXT_PUBLIC_AD_SLOT_LEADERBOARD']} variant="leaderboard" className={cn('hidden md:flex my-6 mx-auto', className)} />;
}

export function AdSlotInContent({ className }: { className?: string | undefined }) {
  return <AdSlot adSlot={process.env['NEXT_PUBLIC_AD_SLOT_IN_CONTENT']} variant="rectangle" className={cn('my-8 mx-auto', className)} />;
}

export function AdSlotInArticle({ className }: { className?: string | undefined }) {
  return <AdSlot adSlot={process.env['NEXT_PUBLIC_AD_SLOT_IN_ARTICLE']} variant="in-article" className={cn('my-6', className)} showLabel={false} />;
}
